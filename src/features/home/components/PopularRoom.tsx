...
export default function PopularRoom({ room }: { room: Room }) {
  ...
  return (
    <div
      className={
        "bg-[#f0f0f0] rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition duration-300 relative"
      }
    >
      ...
      <div className={"p-4 flex flex-col flex-1"}>
        ...
        <div className={"flex gap-2 py-2"}>
          ...
          <h2 className={"text-base text-gray-800"}>
            {room.details.bedSize} Bed
          </h2>
        </div>
        <div className={"flex gap-2 py-2"}>
          <Users />
          <h2 className={"text-base text-gray-800"}>
            {room.guestLimit} Guest{room.guestLimit > 1 ? "s" : ""}
          </h2>
        </div>
        <div className={"mt-auto"}>
          <button
            className={
              "mt-4 w-full bg-blue-900 cursor-pointer hover:bg-blue-800 text-white py-2 px-4 rounded-lg"
            }
          >
            View Details
          </button>
